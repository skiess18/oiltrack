...
class RoutePlanController extends Controller
{
    ...
    {
        $routes = RoutePlan::with('clients')->latest('route_date')->paginate(15);
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($validated['clients'])) {
            ...
            foreach ($validated['clients'] as $i => $clientId) {
                $sync[$clientId] = ['position' => $i + 1, 'visited' => false];
            }
            $route->clients()->sync($sync);
        }

        return redirect()->route('routes.show', $route)
            ->with('success', 'Маршрутът беше създаден успешно.');
    }

    public function show(RoutePlan $route)
    {
        $route->load(['clients' => fn($q) => $q->orderBy('position')]);
    ...
    public function update(Request $request, RoutePlan $route)
    {
        ...
        return redirect()->route('routes.show', $route)
            ->with('success', 'Маршрутът беше обновен успешно.');
    }
    ...
    public function destroy(RoutePlan $route)
    {
        ...
        return redirect()->route('routes.index')
            ->with('success', 'Маршрутът беше изтрит успешно.');
    }
    ...
    public function drive(RoutePlan $route)
    {
        ...
        if (!$client) {
            ...
            return redirect()->route('routes.show', $route)
                ->with('success', '🎉 Маршрутът беше завършен успешно.');
        }
        ...
        return view('routes.drive', compact('route', 'client'));
    }

    public function visit(RoutePlan $route, Client $client)
    {
        $route->clients()->updateExistingPivot($client->id, [
            'visited' => true,
        ]);
    ...
    {
        return redirect()->route('routes.edit', $route)
            ->with('success', '🤖 Оптимизацията ще бъде добавена.');
    }
    ...
    protected function finishOrContinue(RoutePlan $route)
    {
        ...
        return redirect()->route('routes.drive', $route)
            ->with('success', '✅ Обектът беше маркиран като посетен.');
    }
}
