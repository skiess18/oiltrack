...
<div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
    ...
    <div>
        ...
        <h2 class="text-3xl font-bold text-slate-800">
            ...
            ✏️ Редакция на маршрут

        </h2>

        <p class="text-gray-500">

            Редактиране на маршрут за събиране на използвано масло
...
type="date"
name="route_date"
value="{{ old('route_date', optional($route->route_date)->format('Y-m-d') ?? $route->route_date) }}"
...
<option value="">Избери шофьор</option>
...
<option
value="planned"
{{ old('status', $route->status) == 'planned' ? 'selected' : '' }}>
...
<option
value="in_progress"
{{ old('status', $route->status) == 'in_progress' ? 'selected' : '' }}>
...
<option
value="completed"
{{ old('status', $route->status) == 'completed' ? 'selected' : '' }}>
...
<option
value="cancelled"
{{ old('status', $route->status) == 'cancelled' ? 'selected' : '' }}>
...
name="notes"
rows="6"
class="w-full border rounded-xl px-4 py-3">{{ old('notes', $route->notes) }}</textarea>
...
$selectedClients = old(
    'clients',
    $route->clients->pluck('id')->toArray()
);
...
{{ in_array($client->id, $selectedClients) ? 'checked' : '' }}>
